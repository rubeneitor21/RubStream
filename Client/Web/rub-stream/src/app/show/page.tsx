...
export default function Show() {
    ...
    return (
        <main className="bg-background">
            <video controls className="w-screen h-screen bg-backgorund" src={`http://${json.ip}:5000` + videoInfo?.href?.replace("/show?", "")} onTimeUpdate={() => {
                ...
                const video = document.querySelector("video") as HTMLVideoElement
                video.currentTime = videoTime.time || 0
            }}>
            </video>
        </main>
    )
